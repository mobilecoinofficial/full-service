...
#[pymodule]
pub fn transaction_signer_lib(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_wrapped(wrap_pyfunction!(create_account))?;
    m.add_wrapped(wrap_pyfunction!(generate_view_only_import_package))?;
    m.add_wrapped(wrap_pyfunction!(sign_transaction))?;
    m.add_wrapped(wrap_pyfunction!(sync_txos))?;
    Ok(())
}

#[pyfunction]
pub fn create_account(name: &str, mnemonic: Option<&str>) {
...
#[pyfunction]
pub fn generate_view_only_import_package(secret_mnemonic: &str) {
    // Load account key.
    let mnemonic_json =
...
#[pyfunction]
pub fn sync_txos(secret_mnemonic: &str, sync_request: &str, num_subaddresses: u64) {
    // Load account key.
    let mnemonic_json =
        fs::read_to_string(secret_mnemonic).expect("Could not open secret mnemonic file.");
    let account_secrets: AccountSecrets = serde_json::from_str(&mnemonic_json).unwrap();
...
#[pyfunction]
pub fn sign_transaction(secret_mnemonic: &str, sign_request: &str) {
    // Load account key.
    let mnemonic_json =
        fs::read_to_string(secret_mnemonic).expect("Could not open secret mnemonic file.");
    let account_secrets: AccountSecrets = serde_json::from_str(&mnemonic_json).unwrap();
...
pub fn write_json_command_request_to_file(json_command_request: &JsonCommandRequest, filename: &str) {
...
pub fn get_key_images_for_txos(
    tx_outs: &[TxOut],
...
pub fn tx_out_belongs_to_account(tx_out: &TxOut, account_view_private_key: &RistrettoPrivate) -> bool {
...
pub fn generate_subaddress_spend_public_keys(
    account_key: &AccountKey,
