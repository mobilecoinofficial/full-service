...
4. Get the appropriate published enclave measurements, which will be saved to `$(pwd)/consensus-enclave.css`
...
1. _ONLINE MACHINE_: Sync ledger by running full service.
...
1. _ONLINE MACHINE and USB_: Copy the ledger and the full-service binary to USB.
...
1. _OFFLINE MACHINE_: Create a ramdisk to store sensitive material.
...
1. _OFFLINE MACHINE and USB_: Copy the ledger and the full-service binary to the Offline Machine.
...
1. _OFFLINE MACHINE_: Run full service in offline mode.
...
1. _OFFLINE MACHINE_: You can now [create](#create-account) or [import](#import-account) your
...
1. _OFFLINE MACHINE and USB_: To send a transaction, you will a Construct a TxProposal using
...
1. _ONLINE MACHINE and USB_: Copy the tx_proposal to the online machine.

   ```sh
   cp /media/tx_proposal.json ./
   ```

1. _ONLINE MACHINE_: Submit transaction to consensus, using the [`submit_transaction`](#submit-transaction) endpoint.
