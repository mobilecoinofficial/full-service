...
mod tests {
    ...
    // &view_private_key);     let account_id =
    // AccountID::from(&account_key);     let main_public_address =
    // account_key.default_subaddress();     let change_public_address =
    // account_key.change_subaddress();     let mut subaddresses:
    // Vec<(String, u64, String, RistrettoPublic)> = Vec::new();
    //     subaddresses.push((
    ...
    //         let fake_tx_out =
    //             TxOut::new(amount, &main_public_address, &tx_private_key,
    // hint).unwrap();         ViewOnlyTxo::create(
    //             fake_tx_out.clone(),
    //             amount,
    ...
    //     let b58_pub_address =
    //         b58_encode_public_address(&subaddress).expect("Could not encode
    ...
    //     let tx_private_key = RistrettoPrivate::from_random(&mut rng);
    //     let hint = EncryptedFogHint::fake_onetime_hint(&mut rng);
    //     let fake_tx_out = TxOut::new(amount, &main_public_address,
    // &tx_private_key, hint).unwrap();     ViewOnlyTxo::create(
    //         fake_tx_out.clone(),
    //         amount,
