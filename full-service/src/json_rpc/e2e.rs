...
mod e2e {
    ...
    //     let mut rng: StdRng = SeedableRng::from_seed([20u8; 32]);
    //     let (client, _ledger_db, _db_ctx, _network_state) = setup(&mut rng,
    // logger.clone());
    ...
    // account_key["view_private_key"].as_str().unwrap();
    ...
    //     let result = res.get("result").unwrap();
    //     let spent_txo_ids =
    // result.get("spent_txo_ids").unwrap().as_array().unwrap();
    ...
    // b58_encode_public_address(&target_public_address).unwrap(),
    ...
    //     let account_key = secrets.get("account_key").unwrap();
    //     let view_private_key =
    // account_key["view_private_key"].as_str().unwrap();     let body =
    // json!({         "jsonrpc": "2.0",
    //         "id": 1,
    //         "method": "import_view_only_account",
    //         "params": {
    ...
    //     let payments_tx_proposal =
    //         mc_mobilecoind::payments::TxProposal::try_from(&
    // json_tx_proposal). unwrap();     add_block_with_tx_proposal(&mut
    // ledger_db, payments_tx_proposal);
    // manually_sync_view_only_account(&ledger_db, &wallet_db,
    // &view_only_account_id, &logger);     assert_eq!(         ledger_db.
    // num_blocks().unwrap(),         (BASE_TEST_BLOCK_HEIGHT + 2) as u64
    //     );
    ...
    //     // two blocks with random key images. One block with out known key
    // image we'll     // want to use to test whether the txo gets marked as
    // spent.     let spent_key_image = KeyImage::from(rng.next_u64());
    //     let key_images = [
    ...
    //     // create key images again, to ensure that only 1 gets marked as
    // spent:     let mut txouts_with_key_images = Vec::new();
    //     let key_images = [
    //         KeyImage::from(rng.next_u64()),
    //         KeyImage::from(rng.next_u64()),
    //         spent_key_image,
    ...
    //         txouts_with_key_images.push((txout,
    // mc_util_serial::encode(&key_images[index])));     }
    ...
    //         "params": {
    //             "txos_with_key_images": txouts_with_key_images
    //         }
    //     });
