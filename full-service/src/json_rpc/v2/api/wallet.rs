    ...
    FPR: FogPubkeyResolver + Send + Sync + 'static,
{
    ...
    let response = match command {
        ...
        JsonCommandRequest::build_and_submit_transaction {
            ...
            // We always want RTH memos in build_and_submit calls since the assumption here
            // is that they are being called on accounts where we can fully sign
            // a transaction (either locally using the account key or using a
            // hardware wallet).
        ...
        JsonCommandRequest::build_transaction {
            ...
            // We always want RTH memos in build_transaction calls since the assumption here
            // is that they are being called on accounts where we can fully sign
            // a transaction (either locally using the account key or using a
            // hardware wallet).
            let transaction_memo = match payment_request_id {
        ...
        JsonCommandRequest::build_unsigned_transaction {
            ...
            let unsigned_tx_proposal: UnsignedTxProposalJSON = (&service
                .build_unsigned_transaction(
                    ...
                    tombstone_block,
                    max_spendable_value,
                    // TODO - We can check if the account has an account key to sign with, and if
                    // so, we can include an RTH memo.
                    // For hardware-wallet or view-only accounts, we can't include an RTH memo.
                    // For now the behavior is to just not include a memo when doing offline
                    // signing.
