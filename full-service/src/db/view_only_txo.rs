...
//             &RistrettoPublic::from_random(&mut rng),
//         );
//         let fake_tx_out = TxOut::new(value as u64, &public_address,
// &tx_private_key, hint).unwrap();

//         // make sure it fails if no matching account

//         let view_only_account_id = "accountId";

//         let err = ViewOnlyTxo::create(fake_tx_out.clone(), value,
// view_only_account_id, &conn);
...
//             txo_id_hex: txo_id.to_string(),
//             view_only_account_id_hex:
// view_only_account.account_id_hex.to_string(),             txo:
// mc_util_serial::encode(&fake_tx_out),             key_image: None,
...
//         let listed =
//             ViewOnlyTxo::list_for_account(&view_only_account.account_id_hex,
// None, None, &conn)                 .unwrap();
...
//         ViewOnlyTxo::update_key_image(&new_txo.txo_id_hex, &key_image,
// &conn).unwrap();         let found = ViewOnlyTxo::get(&new_txo.txo_id_hex,
// &conn).unwrap();
...
//         // test export_txouts_without_key_image. assuming 2 vo txos without
// key image         // and one with from the tests above

//         let txouts =
//
// ViewOnlyTxo::export_txouts_without_key_image(&view_only_account.
// account_id_hex, &conn)                 .unwrap();
...
//         ViewOnlyTxo::delete_all_for_account(&view_only_account.
// account_id_hex, &conn).unwrap();         let listed =
//             ViewOnlyTxo::list_for_account(&view_only_account.account_id_hex,
// None, None, &conn)                 .unwrap();
